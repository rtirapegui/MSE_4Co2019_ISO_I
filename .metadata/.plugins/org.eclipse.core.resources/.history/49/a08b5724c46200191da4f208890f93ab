 ...
 *	Core functions	*
 *******************/
void OS_CORE_TASK_PREEMPT_DISABLE(void)
{
	/* Nested preempt disable function */

	/* Check preempt counter is 0 */
...
void OS_CORE_TASK_PREEMPT_ENABLE(void)
{
	/* Check if the user is trying to enable preempt when not disabled */
	if(0 < g_preemptCounter)
	{
		g_preemptCounter--;

		/* Only enable preempt when the counter reaches 0 and
		 * the state of the preemption flag is disabled */
...
uint32_t os_core_task_getNextContext(uint32_t currentSp)
{
...
void * os_core_task_getCurrentContext(void)
{
	void * context;
...
void os_core_task_unblock(void * const task)
{
	OS_CORE_TASK_PREEMPT_DISABLE();
	{
		...
		for(uint32_t taskIndex;taskIndex < OS_TASKS_COUNT;taskIndex++)
		{
			...
			{
				/* If task is blocked, set state as READY */
				if(TASK_STATE_BLOCKED == g_controlTaskArr[taskIndex].state)
				{
					g_controlTaskArr[taskIndex].blockedTicks = 0;
...
void os_core_task_delay(const tick_t ticksToDelay)
{
...
void os_task_delay(const tick_t ticksToDelay)
{
	...
	os_core_task_delay(ticksToDelay);
}
